...
export default function Navbar() {
  ...
  const { notifications, unreadCount, loading, markAsRead, markAllAsRead, formatTime } = useNotifications();
                  ...
                  <div className="py-2">
                    {searchResults.map((bluray) => (
                      <button
                        key={bluray.id}
                        onClick={() => {
                          router.push(`/dashboard/blurays/${bluray.id}`);
                          setShowSearchResults(false);
                          setSearchQuery('');
                        }}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gray-900 dark:text-white truncate">{bluray.title}</p>
                          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            {bluray.director && (
                              <>
                                <span>•</span>
                                <span className="truncate">{bluray.director}</span>
                              </>
                            )}
                          </div>
                        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
          <div className="relative" ref={notificationRef}>
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 flex items-center justify-center w-5 h-5 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold rounded-full shadow-lg shadow-red-500/50 scale-110">{unreadCount > 9 ? '9+' : unreadCount}</span>
              )}
            </button>
            ...
            {showNotifications && (
              <div className="fixed sm:absolute right-0 sm:mt-3 left-0 sm:left-auto top-16 sm:top-auto w-full sm:w-96 max-w-md bg-white dark:bg-dark-800 rounded-b-2xl sm:rounded-2xl shadow-2xl border-t sm:border border-gray-200 dark:border-dark-700 z-50 animate-in fade-in slide-in-from-top-2 duration-200 max-h-[70vh] sm:max-h-96 overflow-hidden flex flex-col">
                <div className="flex items-center justify-between p-4 sm:p-5 border-b border-gray-200 dark:border-dark-700 flex-shrink-0">
                  <h3 className="font-semibold text-gray-900 dark:text-white text-base sm:text-lg">{t('notifications.title')}</h3>
                  <button
                    ...
                    notifications.map((notification, index) => (
                      ...
                      >
                        <div className="flex items-start gap-2 sm:gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-xs sm:text-sm ${!notification.read ? 'font-semibold text-gray-900 dark:text-white' : 'font-medium text-gray-700 dark:text-gray-300'} mb-1 line-clamp-2`}>
