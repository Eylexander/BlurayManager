...
import { buildPosterUrl, buildBackdropUrl, cleanProductTitle } from '@/lib/tmdb-utils';
import { TMDBResult, TMDBDetails } from '@/types/tmdb';
import AddTagModal from '@/components/modals/AddTagModal';
import BarcodeScannerModal from '@/components/modals/BarcodeScannerModal';

type MediaType = 'movie' | 'series' | 'tv';
...
const buildBlurayData = (
  ...
  if (type === 'movie') {
    ...
    blurayData.release_year = details.first_air_date && details.last_air_date
      ? parseInt(details.first_air_date.split('-')[0] + '-' + details.last_air_date?.split('-')[0])
      : (year ? parseInt(year) : undefined);
  }
  ...
  export default function AddBlurayPage() {
    ...
    const [selectedBluray, setSelectedBluray] = useState<TMDBDetails | null>(null);
    ...
    const handleBarcodeScanned = async (barcode: string) => {
      ...
      try {
        ...
        toast.loading(t('barcode.lookingUp'));
    ...
    const quickAddResult = async (result: TMDBResult) => {
      ...
      try {
        ...
        const blurayData = buildBlurayData(details, type, purchaseDate, purchasePrice, selectedTags);
        ...
        toast.success(t('add.addedToCollection', { title: details.title || 'Unknown' }));
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="flex gap-4">
            {(selectedBluray.poster_path) && (
              <Image
                src={`https://image.tmdb.org/t/p/w300${selectedBluray.poster_path}`}
                alt={selectedBluray.title || selectedBluray.name || 'Media poster'}
                width={128}
                height={192}
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-bold text-white mb-2">
                {selectedBluray.title || selectedBluray.name}
              </h2>
              <div className="space-y-1 text-gray-300">
                {(selectedBluray.release_date || selectedBluray.first_air_date) && (
                  <p className="text-sm">
                    <span className="font-semibold">{selectedBluray.first_air_date ? t('add.firstAirDate') : t('add.year')}:</span>{' '}
                    {selectedBluray.release_date?.split('-')[0] || selectedBluray.first_air_date?.split('-')[0]}
                  </p>
                )}
                {selectedBluray.director && (
                  <p className="text-sm">
                    <span className="font-semibold">{t('add.director')}:</span> {selectedBluray.director}
                  </p>
                )}
                {selectedBluray.runtime && (
                  <p className="text-sm">
                    <span className="font-semibold">{t('add.runtime')}:</span> {selectedBluray.runtime} {t('add.minutes')}
                  </p>
                )}
                {selectedBluray.genres && selectedBluray.genres.length > 0 && (
                  <p className="text-sm">
                    <span className="font-semibold">{t('add.genres')}:</span> {language === 'fr-FR' ? selectedBluray?.fr?.genres?.map(g => g.name).join(', ') : selectedBluray.genres.map(g => g.name).join(', ')}
                  </p>
                )}
                {selectedBluray.vote_average && (
                  <p className="text-sm">
                    <span className="font-semibold">{t('add.rating')}:</span> {selectedBluray.vote_average.toFixed(1)}/10
                  </p>
                )}
              </div>
              <p className="text-sm text-gray-400 mt-3">{language === 'fr-FR' ? selectedBluray?.fr?.overview : selectedBluray.overview}</p>
            </div>
          </div>

          {/* Season Selection for Series */}
          {type === 'series' && selectedBluray.seasons && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-3">
                {t('add.selectSeasons')}
              </label>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {selectedBluray.seasons
                  .filter(s => s.season_number > 0)
                  .map((season) => (
                    <button
