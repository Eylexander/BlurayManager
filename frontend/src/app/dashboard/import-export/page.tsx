...
export default function ImportExportPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute('download', `bluray-collection-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.failed === 0) {
        toast.success(t('importExport.importSuccess', { count: result.success }));
      } else {
        toast.error(t('importExport.importPartialSuccess', {
      ...
      toast.error(error?.response?.data?.error || t('importExport.importFailed'));
    } finally {
      setImporting(false);
      // Reset file input
      e.target.value = '';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            ...
            <div className="space-y-3 mb-6">
              ...
              />
              <FeatureItem
                icon={<AlertCircle className="w-4 h-4 mt-0.5 text-orange-500" />}
            ...
            <label
              htmlFor="csv-upload"
              className={`block w-full px-4 py-3 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-lg font-medium shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-all duration-300 hover:scale-[1.02] cursor-pointer text-center ${importing ? 'opacity-50 cursor-not-allowed' : ''
                }`}
            >
              {importing ? (
              ...
              <div>
                ...
                <div className="space-y-1 max-h-40 overflow-y-auto">
                  {importResult.errors.map((error, index) => (
                    <p key={index} className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/10 px-3 py-2 rounded">
                      {error}
                    </p>
                  ))}
                </div>
